...
namespace Glueful\Extensions\Archive\Console;
 ...
 * @package Glueful\Extensions\Archive\Console
 */
#[AsCommand(
    name: 'archive:manage',
...
class ManageCommand extends BaseCommand
{
        ...
        try {
            ...
            if ($result->success) {
                ...
                if ((bool) $verifyIntegrity) {
                    $this->verifyArchiveIntegrity((string) $result->archiveUuid);
                }
            } else {
    ...
    private function executeExport(InputInterface $input): int
    {
        ...
        switch ($format) {
            case 'json':
                $this->io->text((string) json_encode($summary, JSON_PRETTY_PRINT));
                break;
            case 'csv':
    ...
    private function performPreArchiveChecks(string $table, \DateTime $cutoffDate): void
    {
        ...
        $freeSpace = $freeSpace !== false ? (int) $freeSpace : 0;
