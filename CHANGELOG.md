...
- Register archive migration paths during provider boot so `migrate:run` sees the
  config-gated archive schema through the same CLI lifecycle used by other extension
  migrations.
